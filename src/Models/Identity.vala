...
public class Envoyer.Models.Container { //@TODO refactor this into something more sane
    public Envoyer.Models.Message message = null;
    public Envoyer.Models.Container parent = null;
    public Gee.LinkedList<Envoyer.Models.Container> children = new Gee.LinkedList<Envoyer.Models.Container> ();
    
    public void add_child (Envoyer.Models.Container child) {
...
public class Envoyer.Models.Identity : GLib.Object {
    ...
    public Gee.Collection<Envoyer.Models.ConversationThread> fetch_threads (string name) {
        ...
        //@TODO move this into a threading class

        var id_table = new Gee.HashMap <string,Container> ();

        foreach (var current_message in messages) {
            //@TODO also take into account Gmail thread ids if present
            Container message_container = null;
            
            if (id_table.has_key (current_message.id)) {
                message_container = id_table[current_message.id];
            } else {
                message_container = new Container ();
                id_table[current_message.id] =  message_container;
            }                
            message_container.message = current_message;
            
            Container previous_references_container = null;
            foreach (var reference in current_message.references) {
                Container container = null;

                if (id_table.has_key (reference)) {
                    container = id_table[reference];
                } else {
                    container = new Container ();
                    id_table[reference] =  container;
                }
        ...
        var root_list = new Gee.ArrayList <Envoyer.Models.Container> ();
        
        foreach (var container in id_table.values) {
            if (container.parent == null) {
                root_list.add (container);
            }
        }
        
        // This sould be where the refactored function should end
        
        // @TODO Import sent messages
        
        // Prune empty containers. 
        foreach (var container in root_list) {
            prune_empty_containers (container);
        }

        // Group root set by subject.
        
        var subject_table = new Gee.HashMap <string,Container> ();
        ...
        foreach (var container in root_list) {
            //@TODO pull subject extraction into its own function
            string subject = null;
            if (container.message != null) {
                subject = container.message.subject;
            } else {
        ...
        foreach (var container in subject_table.values) {
    ...
    private void prune_empty_containers (Envoyer.Models.Container parent) {
